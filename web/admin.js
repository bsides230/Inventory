...
async function loadOrders() {
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('orderViewerList').innerHTML = `<div class="text-red-500 py-4">Failed to load orders.</div>`;
        }
    } catch (e) {
        console.error("Error loading orders", e);
        document.getElementById('orderViewerList').innerHTML = `<div class="text-red-500 py-4">Error loading orders.</div>`;
    }
}

function renderOrders() {
    const listEl = document.getElementById('orderViewerList');
    listEl.innerHTML = '';

    if (ordersData.length === 0) {
        listEl.innerHTML = '<div class="text-center text-[var(--color-text-secondary)] py-8 text-sm">No orders found.</div>';
        return;
    }
    ...
    ordersData.forEach((order, index) => {
        ...
        orderCard.className = `bg-[var(--color-bg-body)] rounded-lg border ${isRush ? 'border-red-500/50' : 'border-[var(--color-border)]'} overflow-hidden`;

        // Order Header (Clickable)
        const header = document.createElement('div');
        header.className = `p-4 cursor-pointer hover:bg-[var(--color-border)] transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3`;
        header.onclick = () => toggleOrderDetails(index);

        header.innerHTML = `
            <div>
                <div class="flex items-center gap-2 mb-1">
                    <span class="font-bold text-sm text-[var(--color-text-primary)]">${order.location_name || order.location_pin}</span>
                ...
                <div class="text-xs text-[var(--color-text-secondary)] flex items-center gap-2">
                    ...
                    ${order.needed_by ? `<span><i data-lucide="calendar" class="w-3 h-3 inline ml-2"></i> Needed by: <span class="text-[var(--color-text-primary)]">${order.needed_by}</span></span>` : ''}
                </div>
            </div>
            <div class="flex items-center gap-4 text-sm font-bold">
                <div class="text-green-500" id="orderTotal-${order.id}">$${totalAmount.toFixed(2)}</div>
                <i data-lucide="chevron-down" id="orderIcon-${order.id}" class="w-5 h-5 text-[var(--color-text-secondary)] transition-transform duration-200"></i>
            </div>
        `;

        // Order Details (Hidden by default)
        const details = document.createElement('div');
        details.id = `orderDetails-${order.id}`;
        details.className = `hidden border-t border-[var(--color-border)] p-4 bg-[var(--color-bg-nav)]`;

        let itemsHtml = '<div class="space-y-2 mb-6">';
                ...
                <div class="flex items-center justify-between gap-3 p-3 bg-[var(--color-bg-body)] rounded border border-[var(--color-border)] hover:border-brand-primary transition-colors">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <label for="check-${order.id}-${itemIdx}" class="flex-1 min-w-0 cursor-pointer ${isChecked ? 'line-through text-[var(--color-text-secondary)]' : 'text-[var(--color-text-primary)]'}">
                            ...
                            <div class="text-xs text-[var(--color-text-secondary)]">${item.quantity} ${item.unit} &bull; ${item.category_id}</div>
                        </label>
                    </div>
                    <div class="flex items-center gap-2 shrink-0">
                        <span class="text-sm font-medium text-[var(--color-text-secondary)]">$</span>
                        <input type="number" step="0.01" min="0" placeholder="0.00" class="admin-input w-24 text-right py-1 px-2" value="${amount}" oninput="debounceUpdateOrderItem('${order.id}', ${itemIdx}, 'amount', this.value)">
                    </div>
                </div>
            `;
        });
        ...
        itemsHtml += `
            <div class="flex justify-end pt-4 border-t border-[var(--color-border)]">
                <button onclick="deleteOrder('${order.user_id}', '${order.id}')" class="text-red-500 hover:text-white hover:bg-red-500 px-4 py-2 rounded transition-colors text-sm flex items-center gap-2">
...
function toggleOrderDetails(index) {
    ...
    const detailsEl = document.getElementById(`orderDetails-${order.id}`);
    const iconEl = document.getElementById(`orderIcon-${order.id}`);
...
// Debounce state modifications to prevent spamming the backend on every keystroke
...
function debounceUpdateOrderItem(orderId, itemIdx, field, value) {
    ...
    if (field === 'amount') {
        ...
        const totalEl = document.getElementById(`orderTotal-${orderId}`);
...
function updateOrderItem(orderId, itemIdx, field, value) {
    ...
    // Checkbox changes should save immediately
    saveOrderState(orderId);
}
...
async function saveOrderState(orderId) {
    ...
    try {
        ...
        if (res.ok) {
            const data = await res.json();
            // Update local order data total from server response
